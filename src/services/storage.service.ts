 ...
 * 여행 플랜 커버 이미지 및 지출 영수증 이미지 업로드/삭제를 담당합니다.
 *
 * 커버 이미지 버킷: cover-images (public)
 *   파일 경로: {userId}/{planId}.{ext}
 *
 * 영수증 버킷: receipts (public)
 *   파일 경로: {userId}/{planId}/{expenseId}.{ext}
 */
...
/**
 * 지출 영수증 이미지를 Storage에 업로드하고 public URL을 반환합니다.
 * cover-images 버킷의 receipts/ 하위 경로를 사용합니다.
 * 경로: receipts/{userId}/{planId}/{expenseId}.{ext}
 */
export async function uploadReceiptImage(
  ...
  const ext = file.type.split('/')[1]?.replace('jpeg', 'jpg') ?? 'jpg'
  const path = `${userId}/${planId}/${expenseId}.${ext}`
